...
def get_noc_energy(self,system):
    ...
    for tensor_sz, loc in zip(sz_list, loc_list):
        if loc == 'off':
            energy = system.energy_per_data_byte_offchip_to_onchip + system.energy_per_data_byte_onchip_to_compute
        elif loc == 'on':
...
def get_a_stationary_access(M,N,K, compute, self , system):
    ...
    total_access = num_loop*(a_reads_per_loop+b_reads_per_loop+c_reads_write_per_loop)
...
def per_core_energy_perforamce(M,N,K,per_core_compute, system , self):
    ...
    a_stationary_access , a_stationary_compute = get_a_stationary_access(M,N,K,per_core_compute , self , system)
    b_stationary_access , b_stationary_compute = get_b_stationary_access(M,N,K,per_core_compute , self , system)
    c_stationary_access , c_stationary_compute = get_c_stationary_access(M,N,K,per_core_compute , self , system)
 
    ## Energy = Element reads *read energy  +    compute time * num PEs *read energy
    total_energy_cost_a = a_stationary_access * per_access_energy + (a_stationary_compute * np.prod(per_core_compute)) * per_compute_energy
    total_energy_cost_b = b_stationary_access * per_access_energy + (b_stationary_compute * np.prod(per_core_compute)) * per_compute_energy
    total_energy_cost_c = c_stationary_access * per_access_energy + (c_stationary_compute * np.prod(per_core_compute)) * per_compute_energy

    energies = np.array([total_energy_cost_a , total_energy_cost_b, total_energy_cost_c])
    performances = np.array([a_stationary_compute , b_stationary_compute, c_stationary_compute])
...
def get_matmul_access( self, system ):
    ...
    energies, performances = per_core_energy_perforamce(M//num_cols, N//num_cols, K//num_rows, per_core_compute, system , self) 
    ...
    if(A_size + 2*B_size + C_size < SRAM):
        ...
        on_chip_memory_access = min(np.ceil(min(A_size,B_size)/SRAM)*max(A_size,B_size) + min(A_size,B_size) ,np.ceil(max(A_size,B_size)/SRAM)*min(A_size,B_size)+max(A_size,B_size)) + C_size
    ...
    between_core_movements = min(A_size + B_size, A_size + C_size, B_size + C_size) * (num_rows-1) * per_sram_change_energy 
